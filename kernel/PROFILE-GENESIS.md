...
1. Draft the profile files **live, during the call**, copying from `profile-templates/` into `profile/` and filling them in: `PROFILE.md` (style, guardrails, anchor), `ABOUT-ME.md` (thin baseline + empty Observed Patterns section), `PROJECTS.md` (empty registry), `STACK.md`, `LEARNING-ME.md` (kernel default; refined by observation later), `LAB.md` (copied **blank** — the system-lab log fills only when the first cross-project infra issue appears), `Noticeboard.md` (copied **blank** — the cross-project channel; fills the first time one project pins a note for another), `Scoreboard.md` (copied **blank** — the metric: projects · sessions · hours; fills itself from the first `/offload`, one row per session — the user does nothing).
1b. **Stamp the kit version.** Write the shared version stamp into the new `profile/STACK.md`:
    `SB kit: [current version — the top released header in CHANGELOG.md, at ROOT's top level] · last update-check: [genesis date]`.
    This is the line the update mechanism reads to know which kit version this install runs and when it last checked the repo (see `kernel/UPDATE.md`). Use the CHANGELOG's current top released version — never hardcode a number here (kernel files are versionless by design).
