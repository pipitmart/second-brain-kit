...
### 4b. Native tools vs bash vs Drive MCP
...
### 4d. Concurrency (multi-machine and same-machine) + the beacon
...
- **Never write an assumed, guessed, or default-filled value into the SSOT or any system file** — dates, times, timezones, IDs, names, numbers. A value earns its place in a file only if it was **(a)** loaded from the profile/project data, **(b)** observed in this session, or **(c)** confirmed by the user. If none hold, **ask — don't fill.** A plausible guess written as fact is exactly the failure mode this rule exists to kill. **Claims count as values:** this binds synthesized claims, not just discrete values — an attribution, a causal statement, or a resolved ambiguity written into the record is a "value" in this sense. If a source is ambiguous (an unresolved pronoun, an unclear referent), resolve it from the source itself or a primary reference doc already in the project, ask, or write it explicitly marked unverified — never resolve it by guess. *(Added 06 Jul 2026 after a live incident: an ambiguous call-transcript pronoun resolved by guess put a wrong attribution into a project SSOT while a primary-source doc that settled it sat unread in the same project folder.)*
