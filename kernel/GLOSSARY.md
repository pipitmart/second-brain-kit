...
**Scoreboard** — `profile/Scoreboard.md`. The metric: **projects · sessions · hours**. A **derived view**, not a second ledger — the per-project counters live in the Project Index (`profile/PROJECTS.md` → `Sessions` + `Hours`), floated up by each project at its own `/offload` (the only session that can read that project's log); the Scoreboard rolls them up and keeps an append-only session history. No number is held twice, so the two cannot drift — if they ever disagree, `PROJECTS.md` wins. *Why it matters:* it's the system's only honest measure of itself, counted from what it already records (no new instrumentation, nothing for the user to do). Counts are raw — **hours is a floor ("hours tracked"), never an estimate, and never multiplied against an invented "without the system" baseline** (kernel §11).
...
**Cite-or-mark** — Kernel §13. The four falsifiable claim-shapes — **absence · presence · actor · status** — render in exactly one of two forms, in chat and in any file a session writes: **`(checked: <file or tool>, <when>)`** or the literal marker **`unverified`**. A claim carrying neither is **malformed on its face**. *Why it matters:* the model asserts absences confidently because absence-from-context is indistinguishable from absence-from-the-world, and by the time the guess is a sentence there is no felt decision point. The tag manufactures one — and it converts the user's job from *knowing the ground truth* (unscalable) to *noticing a missing tag* (no expertise needed). Backed by the Offload auditor: a naked guess hides, a cited guess is falsifiable.

**Offload auditor** — The cold sub-agent spawned at every `/offload` (never at `/orient` — it would tax every session against the latency bar). It re-verifies the session's falsifiable claims **against actual state** — asking *"is this true,"* not *"was this checked"* — over everything the session wrote or is about to write, plus the closeout's surfaced actions. It reports; it never rewrites, and it has authority over nothing that isn't decidable against a reachable file. **Fail-open:** spawn loss runs ~1 in 6, so a failed audit never blocks the offload — it stamps `unaudited` instead.

**`unaudited`** — The literal marker carried by a session's closeout **and** its Session Log row when the Offload auditor did not run. *Why it matters:* roughly one session in six ships unaudited, and every one of them must **say so out loud**. A check that fails quietly is worse than no check — a silent skip re-creates the exact shape the mechanism exists to kill ("an automated check reported nothing, and the human supplied the missing evidence"). It does **not** reach the Scoreboard: that counts work, not confidence.

**Protocol manifest** — A completeness check run at both `/orient` and `/offload`: an enumerated list of what the door **owed** (its mechanical writes — a Session Log row, the `**Totals:**` line, a Ledger row, a beacon, and so on), each confirmed **landed** by a direct re-read before the door is allowed to report itself clean. *Why it matters:* the offload auditor and cite-or-mark both ask *"is this TRUE?"* — nothing asked *"did the protocol do everything it was supposed to?"*, and that gap is exactly what fired repeatedly (a missing Session Ledger row, a stale `**Totals:**` line, a fabricated start-timestamp). `/offload`'s manifest is deterministic and always runs, even when the cold auditor's spawn is lost; `/orient`'s is the same idea, lighter, since orient has no auditor and a live latency bar. Clean → one line — `/orient`'s reads `✅ manifest: N/N confirmed`, `/offload`'s reads `✅ manifest: N/N writes confirmed`. A miss → names exactly what's missing and refuses to say "clean." **Detects and marks, never recovers** — a value never captured is marked `unverified`, not guessed.

**Evidence manifest** — The `## Evidence` section in a Decisions Log's **hot zone**: one line per source, saying what exists and where. A **map, never a copy**; written at `/offload` by the session that learned the fact. *Why it matters:* it fixes the epistemics rather than the discipline. An absence-claim collides with the evidence **passively, at read time**, before it is ever spoken — because the fact is already in context instead of one un-thought-of search away.

**Session beacon** — A line a session posts to `profile/Noticeboard.md → ## Session beacons` at `/orient` Step 6 (*"this project is open"*), cleared at `/offload`. Any session about to write a shared ROOT file reads it first: another project live → re-read before writing, prefer append, never touch a line you don't own. **Advisory — it never blocks work**, and a beacon older than **12 h is presumed dead** (a session that crashed without offloading) and ignored, never deleted by another project. *Why it matters:* ROOT is mounted in every session, so concurrent sessions are all live writers to the same shared files — and until the beacon, **no session knew another was running.** Prose cannot fix that; only a signal can.
