...
- **Trigger is the hot-zone ceiling, not warm length** *(changed 20 Jul 2026, SB Product 0.7-17 — supersedes the old "~250-line" rule).* The line-count trigger assumed the whole file was read every session; since 0.7-14's hot/warm split, the warm trail grows **for free** (costs `/orient` nothing) while the thing that actually costs — the **hot zone** — has its own ceiling. So the old trigger fired on healthy files and stayed silent on unhealthy ones. **`/offload` compacts when the hot zone crosses its `≤8K` token ceiling.** **Pointer-safety check, before any compaction runs:** verify no live hot-zone row (an Active Issue, an Open Action) points into a warm entry about to move to Archive. If one does, relocate that entry's reasoning into a fresh dated Decision entry first (kernel §11 — nothing lost to a tidy-up), then compact.
...
- **The duration stamp, and `unverified` on a missing one** *(0.7-16).* The row's Start time comes from `ORIENT.md` Step 0's first-tool-call clock read — a real observation, never a guess (kernel §11). If that reading was skipped, or the only clock check available happened mid-session rather than at the true start, **the row's Start/`~Hrs` is marked `unverified` (or left as a marked floor) — never backfilled from a mid-session timestamp as if it were the real start.** This is what the `ORIENT.md` completeness manifest catches: it detects the gap and forces the honest marker; it cannot reconstruct a time that was never taken. *(Live failure this closes: a mid-orient clock check was once written into a row as its start time — a fabricated §11 value, corrected after the fact.)*
