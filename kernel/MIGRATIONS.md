...
- **Action:**
  ...
  3. **Skill refresh:** re-upload `help-me.skill` (its description now names three branches, not two) — direct link provided at `UPDATE.md` step 8.
- **Values:** the stamp date = today (observed, not guessed). No other values.
- **Verify:** `profile/STACK.md` carries the `SB kit:` line; local `HELP-ME.md` contains "Branch C"; local `ORIENT.md` contains the update-check sub-step and the bash-mount rule; `kernel/UPDATE.md` and `kernel/MIGRATIONS.md` are present. **DEMAND `/stress-test`** — this block touches a skill (`help-me.skill`) and boot-relevant kernel files, so instance-level proof is required before the update is considered complete; then `/orient` must come back clean.
